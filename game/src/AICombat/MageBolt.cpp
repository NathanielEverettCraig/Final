...
namespace AICombat
{
   /* namespace
    ...
    /*void RegisterMageBoltScript(Canis::App &_app)
    {
        REGISTER_PROPERTY(conf, AiCombat::MageBolt, speed);
        REGISTER_PROPERTY(conf, AiCombat::MageBolt, lifeTime);

        DEFAULT_CONFIG_AND_REQUIRED(conf, AiCombat::MageBolt, Canis::RectTransform, Canis::Sprite2D);

        conf.DrawInspector = [](Editor &_editor, Entity &_entity, const ScriptConf &_conf) -> void
        {
            AiCombat::MageBolt* MageBolt = _entity.GetScript<AiCombat::MageBolt>();
            if (MageBolt != nullptr)
            {
                ImGui::InputFloat(("speed##" + _conf.name).c_str(), &MageBolt->speed);
                ImGui::InputFloat(("lifeTime##" + _conf.name).c_str(), &MageBolt->lifeTime);
            }
        };
    ...
    DEFAULT_UNREGISTER_SCRIPT(conf, MageBolt)

    void MageBolt::Create() {}

    void MageBolt::Ready() {}

    void MageBolt::Destroy() {}

    void MageBolt::Update(float _dt) {
